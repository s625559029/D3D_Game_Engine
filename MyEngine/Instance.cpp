...
bool Instance::LoadObjModel(std::wstring filename,        //.obj filename
	...
	std::vector<InstanceData> & inst)
{
	...
	instBuffDesc.ByteWidth = sizeof(InstanceData) * instanceNum;
	...
	op->d3d11Device->CreateBuffer(&instBuffDesc, &instData, &instanceBuff);
...
void Instance::Clean()
{
	instanceBuff->Release();
}

void Instance::Draw(Camera & cam, cbPerObject & _cbPerObj, int instanceNum)
{
	ObjectsPool * op = ObjectsPool::getInstance();
	...
	for (int i = 0; i < meshSubsets; ++i)
	{
		...
		ID3D11Buffer* vertInstBuffers[2] = { meshVertBuff, instanceBuff };
		...
		op->d3d11DevCon->IASetVertexBuffers(0, 2, vertInstBuffers, strides, offsets);

		//Set the WVP matrix and send it to the constant buffer in effect file
		CXMMATRIX WVP = meshWorld * cam.camView * cam.camProj;
		...
		_cbPerObj.isLeaf = false;        // Tell shaders if this is the leaf instance so it will know to the cbPerInstance data or not
		...
		if (!material[meshSubsetTexture[i]].transparent) {
			...
			op->d3d11DevCon->DrawIndexedInstanced(indexDrawAmount, instanceNum, indexStart, 0, 0);
		}
	}
}